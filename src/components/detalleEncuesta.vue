        ...
        <v-layout row wrap mt-2>
          <v-flex xs4>
            <v-text-field
              ...
              readonly
              box
              :value="moment(detalleEncuesta.date).format('DD-MM-YYYY')"
            ></v-text-field>
          </v-flex>
          <v-flex xs4>
            <v-text-field
              ...
              readonly
              box
              :value="moment(detalleEncuesta.date_end).format('DD-MM-YYYY')"
            ></v-text-field>
          </v-flex>
          <v-flex xs4>
            <v-text-field
              readonly
              box
              :value="formatPrice(detalleEncuesta.price)"
              valor_hora
              prefix="$"
            ></v-text-field>
          </v-flex>
          <v-flex xs12>
            <v-textarea box label="Descripción" :value="detalleEncuesta.description" readonly></v-textarea>
          </v-flex>
          <v-flex xs12>
            <h3>ADJUNTOS</h3>
          </v-flex>
          <v-flex xs12>
            <v-layout>
              <div
                v-for="item in detalleEncuesta.attachments"
                :key="item.id"
                style="cursor:pointer; margin-right:10px; list-style: none;"
              >
      ...
      <v-flex xs8>
        ...
        <v-layout row wrap v-if="!detalleEncuesta.committee_only">
          ...
          <v-flex xs12 mt-4 v-if="!comite">
            ...
            <table class="tabla">
              <thead>
                <tr class="encabezado" style=" background: rgb(4 129 228); color:white;">
                  <th></th>
                  <th>SI</th>
                  <th>NO</th>
                  <th>Total</th>
...
export default {
  ...
  methods: {
        ...
        .then((resp) => {
          if (resp.status === 200) {
            ...
            if (this.detalleEncuesta.committee_only) {
              let residents = resp.data.filter(
                (residente) => residente.committee
              );
              ...
              let calculo = [
                {
                  respuesta: "SI",
                  numero: this.si,
                  porcentaje: this.alicuotaSi,
                },
                {
                  respuesta: "NO",
                  numero: this.no,
                  porcentaje: this.alicuotaNo,
                },
                {
                  respuesta: "SIN VOTAR",
                  numero: this.residentes - this.no - this.si,
                  porcentaje: 100 - this.alicuotaNo - this.alicuotaSi,
                },
              ];
    ...
    download(item) {
      let data =
        this.$store.state.conf.api +
        "/questions-attachments/attachment/" +
    ...
    pdf() {
      ...
      doc.text(
        "Inicio: " + moment(this.detalleEncuesta.date).format("DD-MM-YYYY"),
        15,
        35
      );
      doc.text(
        "Fin: " + moment(this.detalleEncuesta.date_end).format("DD-MM-YYYY"),
        15,
        40
      );
      const file =
        "Detalle de encuesta: " + this.$store.state.admin.condos.name + ".pdf";
      let votos = this.votos.map((voto) => {
        let votacion = voto.accepted ? "SI" : "NO";
        let v = {
          fecha: moment(voto.created_at).format("DD-MM-YYYY"),
      ...
      let resumen = [
        {
          titulo: "Resultado de la votación",
          si: this.si,
          no: this.no,
          total: this.si + this.no,
        },
        {
          titulo: "Total alícuota",
          si: parseFloat(this.alicuotaSi).toFixed(2) + "%",
          no: parseFloat(this.alicuotaNo).toFixed(2) + "%",
          total: parseFloat(this.alicuotaSi + this.alicuotaNo).toFixed(2) + "%",
        },
      ];
      doc.autoTable(this.resumen, resumen, { margin: { top: 50 } });
      doc.text("Votantes", 15, 85, {
        margin: { bottom: 75 },
      });
      doc.autoTable(this.columns, votos, {
    ...
    deleteItem(item) {
      this.$axios
        .delete("/questions-attachments/" + item.id + "?trash=true")
        .then((resp) => {
          if (resp.status === 200) {
    ...
    enviar() {
      ...
      fetch(this.$store.state.conf.api + "/questions-attachments/", {
        ...
        headers: {
          Authorization:
            "Bearer " + localStorage.getItem("bazam-token-control"),
        },
      })
        .then((resp) => {
          if (resp.status === 201) {
