...
export default {
  ...
  methods: {
    ...
    deleteItem(item) {
      this.$axios
        .delete("/deliveries/" + item.id + "?trash=true")
        .then(resp => {
          if (resp.status === 200) {
    ...
    save() {
      console.log(this.selectUsuarios);
      if (this.editedIndex > -1) {
        this.$axios
          .put("/deliveries/" + this.editedItem.id, {
